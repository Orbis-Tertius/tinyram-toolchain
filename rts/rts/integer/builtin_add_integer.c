...
builtin_add_integer__partially_app_2(const struct LexicalScope *scope) {
  ...
  mpz_add(data->value.integer.mpz, op1->value.integer.mpz,
...
builtin_add_integer__partially_app_1(const struct LexicalScope *scope) {
  ...
  data->value.fn.apply = &builtin_add_integer__partially_app_2;
  data->value.fn.scope = scope;

  return data;
}

const struct NFData *builtin_add_integer(const struct LexicalScope *scope) {
  struct NFData *data = (struct NFData *)alloc(sizeof(struct NFData));

  data->type = FunctionType;
  data->value.fn.apply = &builtin_add_integer__partially_app_1;
  data->value.fn.scope = scope;
